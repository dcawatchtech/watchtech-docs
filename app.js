(() => {
  ...
  function iconFor(item) {
    ...
    if (/\.(docx?|odt|rtf|txt|md)$/i.test(name)) return "DOC";
    if (/\.(xlsx?|ods|csv|tsv)$/i.test(name)) return "XLS";
    if (/\.(pptx?|odp)$/i.test(name)) return "PPT";
    if (/\.(jpg|jpeg|png|gif|webp|svg)$/i.test(name)) return "IMG";
    if (/\.(zip|7z|rar|tar|gz)$/i.test(name)) return "ZIP";
    if (/\.(mp4|mov|avi|mkv|webm)$/i.test(name)) return "VID";
    return "FILE";
  }
  ...
  function renderTree() {
    ...
    els.treeCount.textContent = String(rootFolders.length + rootFiles.length);

    const all = document.createElement("div");
    const root = document.createElement("button");
    root.type = "button";
    root.className = `tree-item ${state.currentPath === "" ? "active" : ""}`;
    root.innerHTML = `<span class="tree-caret"></span><span class="tree-icon">⌂</span><span class="tree-name">All documents</span><span class="tree-badge">${state.index.files}</span>`;
  ...
  function cardFor(item) {
    const card = document.createElement(item.kind === "folder" ? "button" : "a");
    card.className = `card ${item.kind === "file" ? "file-card" : "folder-card"}`;
    if (item.kind === "file") {
      card.href = relativePathToUrl(item.path);
      card.target = "_blank";
      card.rel = "noopener";
    ...
    card.innerHTML = `
      ...
      <div class="card-footer">
        ...
        <span class="open-label">${item.kind === "folder" ? "Open →" : "Open ↗"}</span>
      </div>`;
    return card;
  }
