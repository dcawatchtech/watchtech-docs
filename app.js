(() => {
  ...
  function closeViewer() {
    ...
    try { els.viewer.close(); } catch (_) { els.viewer.removeAttribute("open"); }
  ...
  function openFile(item) {
    ...
    els.viewerMeta.textContent = item.export_label || item.mimeType || "Document";
    els.viewerNewTab.onclick = () => openDirect(url);
    els.viewerDownload.href = url;
    ...
    if (category === "pdf") {
      ...
      audio.src = url;
      els.viewerBody.appendChild(audio);
    } else if (category === "video") {
      const video = document.createElement("video");
      video.className = "viewer-media viewer-video";
      video.controls = true;
      video.preload = "metadata";
      video.src = url;
      els.viewerBody.appendChild(video);
    } else if (category === "text") {
      const pre = document.createElement("pre");
      pre.className = "viewer-text";
      pre.textContent = "Loading document…";
      ...
      fetch(url, { cache: "no-store" })
        ...
        .catch((error) => {
          console.error(error);
          pre.textContent = "This file could not be previewed here. Use Open in new tab or Download.";
        });
    } else {
      const panel = document.createElement("div");
      panel.className = "viewer-unsupported";
      panel.innerHTML = `<div class="viewer-unsupported-icon">${escapeHtml(iconFor(item))}</div><h3>Preview not available in the browser</h3><p>${escapeHtml(item.name)}</p><small>${escapeHtml(item.mimeType || item.export_label || "Document")}</small>`;
      els.viewerBody.appendChild(panel);
    }
  ...
  function cardFor(item) {
    const card = document.createElement(item.kind === "folder" ? "button" : "button");
    ...
    if (item.kind === "file") {
      ...
      card.addEventListener("click", () => openPath(item.path));
    }
    ...
    card.innerHTML = `
      <div class="card-top">
        <span class="type-icon">${escapeHtml(iconFor(item))}</span>
      </div>
      <div>
        <div class="card-title">${escapeHtml(item.name)}</div>
        <div class="card-meta">${escapeHtml(count)}</div>
      </div>
      <div class="card-footer">
        <span class="item-count">${item.kind === "folder" ? "Browse folder" : formatDate(item.modifiedTime)}</span>
        <span class="open-label">${item.kind === "folder" ? "Open →" : "Preview →"}</span>
      </div>`;
    return card;
  }
  ...
  function renderContent() {
    ...
    if (!folders.length && !files.length) {
      els.content.innerHTML = `<div class="empty-state"><strong>This folder is empty</strong>There are no synchronized items here yet.</div>`;
    }
    renderBreadcrumbs();
    renderTree();
    els.searchResults.hidden = !state.search;
  }
  ...
  function renderSearch() {
    ...
    if (!results.length) {
      els.resultsGrid.innerHTML = `<div class="empty-state"><strong>No matches found</strong>Try another document or folder name.</div>`;
    } else {
      results.sort(sortItems).forEach((item) => els.resultsGrid.appendChild(cardFor(item)));
    }
  }
  ...
  function setView(view) {
    state.view = view;
    localStorage.setItem("watchtech-view", view);
  ...
  function toggleTheme() {
    ...
    localStorage.setItem("watchtech-theme", next);
  ...
  function updateConnection() {
    ...
    els.connectionText.textContent = online ? "Online" : "Offline mode";
    els.connectionDot.style.background = online ? "var(--success)" : "#e7a13d";
  }

  async function getCache() {
    if (!("caches" in window)) return null;
    return caches.open("watchtech-docs-v1");
  }
  ...
  async function cacheAllDocs() {
    ...
    for (const item of files) {
      try {
        const response = await fetch(relativePathToUrl(item.path), { cache: "no-store" });
        if (response.ok) await cache.put(relativePathToUrl(item.path), response.clone());
      } finally {
        done += 1;
        els.offlineMeta.textContent = `${done} / ${files.length} documents`;
      }
    }
    els.offlineText.textContent = "Offline library ready";
    els.offlineMeta.textContent = `${files.length} documents cached on this device`;
    showToast("WatchTech Docs is ready for offline use");
  }

  async function checkOfflineCache() {
    const cache = await getCache();
    if (!cache || !state.index) return;
    const keys = await cache.keys();
    const filePaths = state.index.items.filter((i) => i.kind === "file").map((i) => relativePathToUrl(i.path));
    const cached = keys.filter((request) => filePaths.includes(request.url)).length;
    if (cached > 0) {
      els.offlineText.textContent = "Offline library ready";
      els.offlineMeta.textContent = `${cached} / ${filePaths.length} documents cached`;
    }
  }
  ...
  function initEvents() {
    els.searchInput.addEventListener("input", (event) => {
      ...
      renderSearch();
      if (state.search) els.searchResults.scrollIntoView({ behavior: "smooth", block: "start" });
    });
    ...
    window.addEventListener("keydown", (event) => {
      ...
      if (event.key === "Escape" && document.activeElement === els.searchInput) {
        els.searchInput.value = ""; state.search = ""; renderSearch(); els.searchInput.blur();
      }
    });
  }
