...
def save_library_index(current: dict[str, dict[str, Any]]) -> None:
    """Publish a small public index for the static PWA to consume."""
    DOCS_ROOT.mkdir(parents=True, exist_ok=True)
    index_path = DOCS_ROOT / ".watchtech-library.json"
    items = []
    for item in current.values():
        ...
        if item["kind"] == "file":
            record["export_label"] = item.get("export_label")
        items.append(record)
    items.sort(key=lambda item: (item["kind"] != "folder", item["path"].lower()))
