(() => {
  ...
  const DOC_CACHE = "watchtech-docs-v5";
  const state = {
    index: null,
    currentPath: "",
    search: "",
  ...
  async function openFile(item) {
    ...
    els.viewerNewTab.onclick = () => window.open(url, "_blank", "noopener,noreferrer");
    const category = fileCategory(item);
    if (category === "pdf") els.viewerBody.innerHTML = `<iframe class="viewer-frame" src="${escapeHtml(url)}" title="${escapeHtml(item.name)}"></iframe>`;
    else if (category === "image") els.viewerBody.innerHTML = `<div class="viewer-media-wrap"><img class="viewer-image" src="${escapeHtml(url)}" alt="${escapeHtml(item.name)}"></div>`;
    else if (category === "audio") els.viewerBody.innerHTML = `<div class="viewer-media-wrap"><audio class="viewer-audio" controls autoplay src="${escapeHtml(url)}"></audio></div>`;
    else if (category === "video") els.viewerBody.innerHTML = `<div class="viewer-media-wrap"><video class="viewer-video" controls autoplay src="${escapeHtml(url)}"></video></div>`;
    else if (category === "text") {
      viewerShellMessage("Loading text…", "");
      try {
        const response = await fetch(url, { cache: "no-store" });
        if (!response.ok) throw new Error(`HTTP ${response.status}`);
        const text = await response.text();
        els.viewerBody.innerHTML = `<pre class="viewer-text">${escapeHtml(text)}</pre>`;
      } catch (error) { console.error(error); viewerShellMessage("Preview unavailable", "Use Download to open this file with its normal application."); }
    } else viewerShellMessage("Browser preview not available", "Use Download or Open separately for this file type.");
